...
SELECT shops.id, shops.shopname, cats.name, cats.id
...
SELECT  shops.shopname
FROM shops inner join 
(SELECT shops_id
FROM cats
WHERE cats.name='MURZIK'
 ) as c on  shops.id = c.shops_id;
 
SELECT  shops.shopname
FROM cats inner join shops on  shops.id = cats.shops_id
WHERE cats.name not in ("Murzik", "Zuza"); 

SELECT analysis.an_name, analysis.an_cost
FROM  Orders inner join analysis on analysis.an_id= Orders.ord_an
WHERE Orders.ord_datetime between 20200205 and 20200213;